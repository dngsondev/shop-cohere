...
function AdminTable({ admins, loading, onView, onEdit, onDelete, onToggleStatus }) {
    if (loading) {
        return (
                            ...
                            <tr key={admin.admin_id} className="hover:bg-gray-50">
                                ...
                                <td className="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                                    <div className="flex items-center justify-end space-x-2">
                                        ...
                                        <button
                                            onClick={() => onToggleStatus && onToggleStatus(admin.admin_id, admin.status)}
                                            className={`p-1 ${admin.status === 1 ? 'text-red-600 hover:text-red-900' : 'text-green-600 hover:text-green-900'}`}
                                            title={admin.status === 1 ? 'Vô hiệu hóa' : 'Kích hoạt'}
                                        >
                                            {admin.status === 1 ? (
                                                <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
